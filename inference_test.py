...
def face_detect(images):
    ...
    batch_size = 2#args.face_det_batch_size
...
mel_step_size=16
video_root = 'data_root\\main\\Test4\\video'
audio_root = 'data_root\\main\\Test4\\audio'
audio_file = glob(os.path.join(audio_root, '*.wav'))[0]
video_file = glob(os.path.join(video_root, '*.mp4'))[0]
#video_file = '/content/drive/MyDrive/motion_model+lip_sync/motion_model.mp4'
print(video_file)
...
#mel=librosa.feature.melspectrogram(y=wav, sr=16000, n_fft=800, hop_length=200, win_length=800, n_mels=80, fmin=55, fmax=7600)
print(mel.shape)
...
while 1:
        ...
        S.append([start_idx, start_idx+mel_step_size])
...
for i in range(len(mel_chunks)):
  if i<10:
    j=0
    print('J=',j)
    full_frames_1.append(full_frames[j])
  else:
    j=j+1 
    full_frames_1.append(full_frames[j])
print("Length of frames: {}".format(len(full_frames_1)))
...
for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
    ...
    if i == 0:
            ...
            out = cv2.VideoWriter('temp\\result_delayed.mp4', 
    ...
    for p, f, c in zip(pred, frames, coords):
            ...
            out.write(f)
            #cv2.imwrite(os.path.join('/content/drive/MyDrive/Wav2Lip-master/temp','{}.jpg'.format(1)), p)
print("Running time in seconds",(time.time() - start_time))
out.release()